...
export type FinalCTAProps = {
  /** Defaults to `final-store` (homepage anchor). Use a unique id when embedding on other pages. */
  anchorId?: string;
  /** `aria-labelledby` target — must be unique per page when multiple Final CTAs exist. */
  titleId?: string;
};

export default function FinalCTA({
  anchorId = 'final-store',
  titleId = 'final-quiet-title',
}: FinalCTAProps) {
  ...
  return (
    <section
      ref={ref}
      id={anchorId}
      className="final-cta home-section-final section-fade-in"
      ...
      <div className="container container--narrow">
        <div className="final-cta-content">
          ...
          <div className="final-cta-buttons final-cta-buttons--final">
            <a
              href="https://apps.apple.com/app/id6754100684"
              target="_blank"
              rel="noopener noreferrer"
              className="btn-final-store"
              aria-label={t('appStore')}
            >
              <svg
                className="btn-final-store__icon"
                width={20}
                height={20}
                viewBox="0 0 24 24"
                fill="none"
              ...
              <span className="btn-final-store__label">{t('appStore')}</span>
            </a>
            <a
              href="https://play.google.com/store/apps/details?id=now.inigo.app"
              target="_blank"
              rel="noopener noreferrer"
              className="btn-final-store"
            ...
            >
              ...
              >
                <path
                  d="M3 20.5V3.5C3 2.91 3.34 2.39 3.84 2.15L13.69 12L3.84 21.85C3.34 21.6 3 21.09 3 20.5ZM16.81 15.12L6.05 21.34L14.54 12.85L16.81 15.12ZM20.16 10.81C20.5 11.08 20.75 11.5 20.75 12C20.75 12.5 20.5 12.92 20.16 13.19L17.19 15.12L14.54 12.85L17.19 10.68L20.16 10.81ZM6.05 2.66L16.81 8.88L14.54 11.15L6.05 2.66Z"
                  fill="currentColor"
                />
              </svg>
              <span className="btn-final-store__label">{t('googlePlay')}</span>
            </a>
          </div>
        </div>
      </div>
